...
class Network {
    ...
    private func dayCleaner(forcastedWeatherdayByHours hours: [ForcastedWeatherDay]) -> ForcastedWeatherDay {
        ...
        let day = ForcastedWeatherDay(weather: hours[0].weather, temp: tempTotal, feelsLike: feelsLike, tempMin: tempLow!, tempMax: tempHigh, cloudPercentage: clouds, date: hours[0].date)
